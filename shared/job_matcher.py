"""
shared/job_matcher.py — AI job matching using FREE Google Gemini API
Works for all platforms: LinkedIn, Indeed, Internshala, Freelancer.
...
def score_jobs(jobs: list, resume_text: str, config: dict) -> list:
    ...
    for i, job in enumerate(jobs):
        try:
            ...
            job["reason"] = reason
            scored.append(job)
            logger.info(f"   [{i+1}/{len(jobs)}] {job['title']} @ {job.get('company','N/A')} → {score}/100")
...
def _score_job(api_key: str, job: dict, resume_text: str, config: dict) -> tuple:
    ...
    # Freelancer projects have different fields
